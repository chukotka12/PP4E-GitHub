...
def consumer(root):
    try:
        ...
        root.see('end')
    root.after(250, lambda: consumer(root))  # 4 раза в сек.
...
if __name__ == '__main__':
    ...
    root = ScrolledText()
    root.pack()
    root.bind('<Button-1>', lambda event: maketreads())
    consumer(root)  # запуск цикла проверки очереди в главном потоке окна
