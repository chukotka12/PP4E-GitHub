...
схем запуска;
автоматически вставляет “python” и/или путь к выполняемому файлу
...
def fixWindowsPath(cmdline):
    ...
    return ' '.join([fixedpath] + splitline[1:])  # снова объединить
...
class StartArgs(LaunchMode):
    ...
    def run(self, cmdline):
        assert sys.platform[:3] == 'win'
        os.system('start ' + cmdline)  # может создавать окно консоли
