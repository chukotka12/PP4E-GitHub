...
if len(argv)>1: dirname=argv[1]     # пример: C:\, C:\Python-3.7\Lib
if len(argv)>2: extname=argv[2]     # пример: .pyw, .txt
if len(argv)>3: trace=int(argv[3])  # пример: . .py 2
...
for (thisDir, subsHere, filesHere) in os.walk(dirname):
    ...
    if fixname in visited:
        if trace: tryprint('scipping '+thisDir)
    else:
        visited.add(fixname)
