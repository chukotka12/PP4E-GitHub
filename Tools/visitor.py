"""
##############################################################################
 Использует классы и подклассы для сокрытия деталей использования функции os.walk при
...
переопределите метод reset для поддержки множественных, независимых
объектов-обходчиков, требующих обновлений в подклассах;
...
class SearchVisitor(FileVisitor):
    ...
    # skipexts = ['.gif', '.jpg', '.pyc', '.o', '.a', '.exe'] # игнорируемые расширения
    skipexts = []
    ...
    def visitfile(self, fname):
        ...
        if not self.candidate(fname):
            ...
            text = open(fname, encoding='UTF-8').read()  # ‘rb’ для недекодируемого текста или text.find() != -1
